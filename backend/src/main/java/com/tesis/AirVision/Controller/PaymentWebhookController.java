...
public class PaymentWebhookController {
    ...
    public ResponseEntity<String> webhook(
        ...
        if (id == null || "null".equals(id)) {
            log.warn("⚠️ Webhook sin ID válido, ignorado");
        ...
        try {
            if ("payment".equalsIgnoreCase(topic)) {
                ...
                log.info("ℹ️ Tipo de webhook no procesado: {}", topic);
    ...
    private void processPaymentDirect(Long paymentId) {
        ...
        try {
            ...
            if ("approved".equalsIgnoreCase(status)) {
                ...
                log.info("ℹ️ Pago no aprobado (status={}), no se actualiza el rol", status);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if ("paid".equalsIgnoreCase(orderStatus)) {
                if (order.getPayments() != null && !order.getPayments().isEmpty()) {
                    ...
                    log.warn("⚠️ Orden paid pero sin pagos asociados");
                }
            } else {
                log.info("ℹ️ Orden no está 'paid' (estado: {}). No se actualiza el rol.", orderStatus);
            }
    ...
    private void updateUserToPremium(String userId, Payment payment) {
        if (userId == null || userId.isEmpty()) {
            log.warn("⚠️ No hay userId (externalReference) en el pago");
            return;
        }
        ...
        try {
            ...
            if (opt.isPresent()) {
                ...
                if (user.getRole() != Role.USER_PREMIUM) {
                    ...
                    log.info("ℹ️ Usuario {} ya es PREMIUM (webhook duplicado)", user.getEmail());
                }
            } else {
